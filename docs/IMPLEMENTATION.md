...
- **Editable transcripts** (Meetily #377) — conflicts with "transcript.json is machine output"; needs a provenance design. **Now designed and phased as P4 in [PRIVACY.md](PRIVACY.md) §4.1**: edits go to a sibling `corrections.json` keeping the original text, never into `transcript.json`. The decisive argument is that re-transcription rewrites `transcript.json` wholesale, so anything written there is by design disposable.
...
- **Speaker attribution across the two tracks** — researched and phased in [ATTRIBUTION.md](ATTRIBUTION.md). Both reported bugs are now fixed: A1 replaced the raw-dB level test that was deleting genuine near-end speech (it measured channel gain, not bleed), and A3 shortened the VAD region so an interjection sorts into the middle of a long turn rather than after it. Two findings there are worth reading before touching either: **per-segment normalization cannot work** — it inverts the classes, so the fix is a per-recording gate — and **`maxSpeechDuration` is a target rather than a ceiling**, since Silero never cuts mid-speech. A2 (cross-channel correlation) remains open, now scoped to the single case the gate declines to judge: a recording with both a near-end speaker *and* bleed.
- **In-meeting mute, deleting recordings, and vault encryption** — researched in [PRIVACY.md](PRIVACY.md), now **phased as P1–P5** there. Decisions taken: build delete (P1) and Oratio's own mute button (P2), replace the audio-retention boolean with a Fireflies-style mode defaulting to transcript-only (P3), make transcripts editable via an overlay file (P4), and **build no encryption at all** (P5) — Logseq shipped exactly that feature and removed it as "too intrusive to the core". Note P3 ships *after* P4 deliberately: discarding audio removes the recording a correction is checked against. Three findings decide the rest. **Deleting a recording is already fully built** — `deleteSession`, its IPC handler and the preload bridge all exist and are correct; only a button is missing. **Reading another app's mute state is impossible on macOS**, verified rather than assumed: `AVAudioApplication.isInputMuted` is strictly per-process (measured — a second process never sees it), Core Audio process objects expose no mute property at all (0/26), and only the Accessibility permission can do it, which costs more than the bug. **Vault encryption mostly defends threats the OS already covers** — FileVault handles disk theft, POSIX handles other accounts, and the one real gap (an unlocked machine) is not closed by encryption that auto-unlocks at login. If a feature ships there, it is an app-level *lock*, not encryption, or an encrypted sparse bundle — never a custom file format, which is the Granola mistake.
